...
public function viewShortlist()
	{
			 ...
			 return $this->db->select('count(DISTINCT(stl.can_short_id)) as can_short_id,stl.*,bi.can_id,bi.first_name,bi.last_name')   
...
public function viewInterview()
	{
			 return $this->db->select('count(DISTINCT(int.can_int_id)) as can_int_id,int.*,bi.can_id,bi.first_name,bi.last_name')   
	...
	public function interview_updateForm($id){
        $this->db->where('can_int_id',$id);
        $query = $this->db->get('candidate_interview');
        return $query->row();
    }
	...
	public function selection_updateForm($id){
        $this->db->where('can_sel_id',$id);
        $query = $this->db->get('candidate_selection');
        return $query->row();
    }
    ...
    public function dropdowninterview()
	{
        ...
        $list = array();
       	if (!empty($data) ) {
       		foreach ($data as $value) {
       			$list[$value->can_id] = $value->can_id;
       		} 
       	}
       	return $list;
	}
