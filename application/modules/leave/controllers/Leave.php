...
public function application()
    { 
        ...
        if ($this->form_validation->run() === true) {
           ...
           //$records = $this->db->query(" SELECT count(DISTINCT(ap.leave_appl_id)) as leave_appl_id, `ap`.*, `p`.`employee_id`, `p`.`first_name`, `p`.`last_name` FROM `leave_apply` `ap` LEFT JOIN `employee_history` `p` ON `ap`.`employee_id` = `p`.`employee_id` GROUP BY `ap`.`leave_appl_id` ORDER BY `ap`.`leave_appl_id` DESC");
            
              // $data['mang']   =  $records->result_array();
