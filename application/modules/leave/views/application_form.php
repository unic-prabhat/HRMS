    ...
    <div class="col-sm-12">
        ...
        <div class="panel panel-default thumbnail"> 
            ...
            <div class="panel-body">
                                <?php
        //         $this->db->select('count(DISTINCT(ap.leave_appl_id)) as leave_appl_id,ap.*,p.employee_id,p.first_name,p.last_name')   
        //     ->from('leave_apply ap')
       ...
       // $this->db->select('*')->from('leave_apply');
        ...
        //$hello = $this->db->query("SELECT count(DISTINCT(ap.leave_appl_id)) as leave_appl_id, `ap`.*, `p`.`employee_id`, `p`.`first_name`, `p`.`last_name` FROM `leave_apply` `ap` LEFT JOIN `employee_history` `p` ON `ap`.`employee_id` = `p`.`employee_id` GROUP BY `ap`.`leave_appl_id` ORDER BY `ap`.`leave_appl_id` DESC");
        
        //$mang= $hello->result_array();
        // print_r($mang);
            
         // echo  $this->db->last_query();
            
            ?>
                <table stywidth="100%" class="datatable table table-striped table-bordered table-hover">
                    <thead>
                        <tr>
